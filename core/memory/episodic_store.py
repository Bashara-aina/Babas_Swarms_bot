...
class EpisodicStore:
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            ...
            logger.info("[EpisodicStore] SQL: CREATE TABLE legion_episodes (id uuid PRIMARY KEY, user_id text, episode_type text, summary text, detail text, tags jsonb, ts float8, source text, metadata jsonb);")
    ...
    def store(self, user_id: str, episode_type: str, summary: str,
              detail: str = "", tags: list[str] | None = None,
              metadata: dict | None = None, source: str = "user") -> str:
    ...
    def recall(self, user_id: str, query: str, limit: int = 8,
               episode_type: str | None = None) -> list[dict]:
    ...
    def _supabase_recall(self, user_id: str, query: str, limit: int,
                         episode_type: str | None) -> list[dict]:
        q = self._supabase.table("legion_episodes").select(
            "id,episode_type,summary,detail,tags,ts,source,metadata"
        ).eq("user_id", user_id).order("ts", desc=True).limit(limit * 3)
        if episode_type:
    ...
    def _local_recall(self, user_id: str, query: str, limit: int,
                      episode_type: str | None) -> list[dict]:
