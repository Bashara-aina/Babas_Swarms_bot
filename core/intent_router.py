...
_PATTERNS: dict[Intent, list[str]] = {
    Intent.COMPUTER_CONTROL: [
        ...
        r"\bwindow\b",
        r"\bdesktop\b",
        r"\bplay\b.*\b(spotify|youtube|vlc|music)\b",
        r"\bvolume\b",
        r"\bbuka\b",
        r"\btutup\b.*\bapp\b",
    ],
    ...
    Intent.WEB_SCRAPE: [
        r"\bscrape\b",
        r"\bextract.*from\b.*http",
        r"\bget.*data.*from\b.*http",
        r"\bparse\b.*\bhttp\b",
        r"\bwebsite.*data\b",
        r"\bget.*from.*url\b",
    ],
    Intent.MEMORY_SEARCH: [
        r"\bwhat did i.*say\b",
        r"\bwhat did i.*\btell\b",
        r"\bdo you remember\b",
        r"\brecall\b",
        r"\blast.*time\b.*\b(i|we)\b",
        r"\bpreviously\b",
        r"\byou told me\b",
        r"\bi mentioned\b",
        r"\bkamu ingat\b",
    ...
    Intent.SCHEDULE_TASK: [
        r"\bremind\b",
        r"\bset.*reminder\b",
        r"\bschedule\b.*\btask\b",
        r"\btomorrow\b.*\bat\b\s*\d",
        r"\bset.*alarm\b",
        r"\bevery (day|week|morning|night)\b",
        r"\bcron\b",
        r"\bingatkan\b",
        r"\bjadwalkan\b",
    ],
    ...
    Intent.SITE_ANALYSIS: [
        r"\brumahlabuh\b",
        r"\bmy.*site\b.*\b(slow|down|broken|check)\b",
        r"\bwebsite.*performance\b",
        r"\blighthouse\b",
        r"\bpage.*speed\b",
        r"\bseo\b",
        r"\banalyze.*site\b",
    ...
    Intent.DATABASE_AUDIT: [
        r"\bsupabase\b",
        r"\bdatabase\b.*\b(check|audit|schema|query)\b",
        r"\btable\b.*\b(schema|structure|check)\b",
        r"\bsql\b.*\bquery\b",
        r"\bdb\b.*\b(check|audit)\b",
        r"\brows\b.*\btable\b",
    ],
    ...
    Intent.LOCATION_QUERY: [
        ...
        r"\bwhere.*eat\b",
        r"\bfood.*near\b",
        r"\bdirections?\b.*\bto\b",
        r"\bnearby\b",
        r"\bnear me\b",
        r"\bnear\b.*\b(here|koto|tokyo|shibuya|shinjuku)\b",
        r"\brecommend.*place\b",
        r"\bgood.*\b(food|place|spot)\b.*\bnear\b",
        r"\bkafe\b",
        r"\bmakan.*dimana\b",
        r"\btempat.*rekomen\b",
    ...
    Intent.TRANSLATION: [
        r"\btranslate\b",
        r"\bin\b.*(english|indonesian|japanese|bahasa)\b",
        r"\bterjemahkan\b",
        r"\bartinya apa\b",
        r"\bke bahasa\b",
    ],
    ...
    Intent.DATA_ANALYSIS: [
        r"\banalyze.*data\b",
        r"\b(csv|json|excel)\b.*\banalyze\b",
...
def build_intent_hint(result: IntentResult) -> str:
    ...
    if result.intent == Intent.CASUAL_CHAT or result.confidence < 0.65:
        return ""
    parts = [f"[Detected intent: {result.intent.value} — confidence {result.confidence:.0%}."]
...
async def classify_intent_llm(message: str) -> IntentResult:
    """
    LLM-powered intent classification for ambiguous messages (confidence < 0.7).
...
- If nothing fits well → "casual_chat"
- Return ONLY the intent name in lowercase with underscores
...
Respond with ONLY the intent name, nothing else."""
        ...
        for intent in Intent:
            if intent.value == raw or intent.value.replace("_", " ") == raw:
        ...
        intent_out = matched_intent or Intent.CASUAL_CHAT
        return IntentResult(
            intent=intent_out,
            confidence=0.85,
            method="llm",
            raw_message=message,
            suggested_agent=_INTENT_TO_AGENT.get(intent_out, "general"),
            needs_tools=intent_out in _INTENT_NEEDS_TOOLS,
            needs_research=intent_out in _INTENT_NEEDS_RESEARCH,
        )
    except Exception as e:
        logger.debug("[IntentRouter] LLM classification failed: %s", e)
        return IntentResult(
            Intent.CASUAL_CHAT,
            0.5,
            "llm_fallback",
...
class IntentRouter:
    """High-level intent routing interface wrapping the classify_intent pipeline."""

    def __init__(self) -> None:
        pass

    async def route(self, message: str) -> IntentResult:
        """Classify intent with LLM fallback for ambiguous messages."""
        return await classify_intent(message)

    def route_sync(self, message: str) -> IntentResult:
        """Synchronous pattern-only classification (for hot paths)."""
        return classify_intent_fast(message)
