...
class DifyClient:
    ...
    async def run_workflow(self, workflow_id: str, inputs: dict, user_id: str = "legion") -> dict:
        ...
        try:
            ...
            return {"output": f"Dify unavailable: {e!s}", "status": "error"}

    async def chat(
        self, app_id: str, message: str, conversation_id: str | None = None, user_id: str = "legion"
    ) -> dict:
        if not self.available:
    ...
    async def health_check(self) -> bool:
        try:
            async with aiohttp.ClientSession() as session, session.get(f"{self.base_url}/health", timeout=aiohttp.ClientTimeout(total=5)) as resp:
