...
This wiki/ is an Obsidian vault containing synthesized project knowledge. All sessions that touch wiki/ must follow this protocol.
...
STEP 1 — Verify Obsidian vault is pointed at the right directory:
  ...
  Correct vault root is: ~/swarm-bot/wiki/ — NOT: ~/swarm-bot/ (root) — NOT: ~/swarm-bot/.wiki/ (DEPRECATED)
...
PATH RULES (absolute — never deviate):
  ...
  NEVER TO:    ~/swarm-bot/.wiki/       ❌ (same, absolute path version)
