...
5a. obsidian: read_note("vault-manifest.json") → load vault structure
   → If missing: run om-vault-audit to initialize brain/work/org/perf structure
...
| TIER5 STRUCTURAL | .wiki/ Obsidian + obsidian-mind | obsidian_search_notes | obsidian_create_note | permanent |

**obsidian-mind vault structure** (supplements TIER5):
- `brain/` — Long-term memory: North Star, Memories, Key Decisions, Patterns, Gotchas, Skills
- `work/` — Projects: active/ (current), archive/ (completed), incidents/, 1-1/
- `org/` — People & teams: People & Context, people/, teams/
- `perf/` — Performance: Brag Doc, competencies/, evidence/, cycles/
- `templates/` — Standard formats: work-note, decision, thinking, competency, 1-1, incident
- `reference/` — External: articles, docs, resources
- `thinking/` — Exploratory: 4-phase analyze→hypothesize→implement→verify

**obsidian-mind commands** (slash commands in .claude/commands/):
- `/om-standup` — Morning kickoff: Git log review, active incidents, today's plan
- `/om-dump` — Brain dump routing to brain/Memories.md
- `/om-wrap-up` — Session end: update work-notes, dump memories, commit wiki
- `/om-weekly` — Week review: archive work-notes, update Brag Doc, prep weekly
- `/om-incident-capture` — Incident post-mortem → work/incidents/
- `/om-prep-1on1` — Manager 1-1 prep from org/people data
- `/om-vault-audit` — Health check: orphan notes, broken links, missing metadata
