...
**Action:** Train PSR with `KENDALL_FIXED_WEIGHTS=1` env var and fixed manual weights, removing the Kendall suppression. Run for 5-10 epochs from epoch_18 checkpoint.
...
**Files:**
- Environment — launch with `KENDALL_FIXED_WEIGHTS=1` env var
...
**Action:** Run training with `KENDALL_FIXED_WEIGHTS=1` env var, KENDALL_HP_FIXED_LAMBDA=0.2. Compare PSR F1.
...
**Files:**
- Environment — `KENDALL_FIXED_WEIGHTS=1` env var
