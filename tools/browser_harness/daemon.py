...
def get_ws_url() -> str:
    ...
    if url := os.environ.get("BU_CDP_URL"):
        ...
        while time.time() < deadline:
            try:
                return json.loads(
                    urllib.request.urlopen(f"{url}/json/version", timeout=5).read()
                )["webSocketDebuggerUrl"]
            except Exception as e:
                last_err = e
                time.sleep(1)
        raise RuntimeError(
            f"BU_CDP_URL={url} unreachable after 30s: {last_err} -- "
        ...
        while True:
            ...
            try:
                ...
                if time.time() >= deadline:
                    raise RuntimeError(
                        f"Chrome's remote-debugging page is open, but DevTools is not live yet "
                        f"on 127.0.0.1:{port.strip()} — if Chrome opened a profile picker, "
                        "choose your normal profile first, then tick the checkbox and click Allow if shown"
                    )
                time.sleep(1)
            finally:
                probe.close()
    ...
    for probe_port in (9222, 9223):
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{probe_port}/json", timeout=2) as r:
            ...
            pass
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{probe_port}/json/version", timeout=2) as r:
...
class Daemon:
    ...
    async def attach_first_page(self):
        if "/page/" in self.cdp.ws_url:
            log("page-level WS — enabling domains directly; browser session cleared for Target.*")
            for d in ("Page", "DOM", "Runtime", "Network"):
                try:
                    await asyncio.wait_for(self.cdp.send_raw(f"{d}.enable"), timeout=5)
                except Exception as e:
                    log(f"enable {d}: {e}")
            self.session = None
        ...
        self._browser_session = (await self.cdp.send_raw(
            "Target.attachToTarget", {"targetId": pages[0]["targetId"], "flatten": True}
        ))["sessionId"]
        self.session = self._browser_session
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if os.environ.get("BU_CDP_WS"):
                raise RuntimeError(
                    ...
                    "or the remote endpoint is down. If you use Browser Use cloud, verify BROWSER_USE_API_KEY "
        ...
        self.cdp.set_event_callback(on_event)
    ...
    async def handle(self, req: dict) -> dict:
        ...
        if meta == "browser_session":
            return {"session_id": self._browser_session}
        if meta == "shutdown":
            self.stop.set()
            return {"ok": True}
